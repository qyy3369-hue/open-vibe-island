...
| **Antigravity** | Supported | Global JSON hooks track agent work and confirm completion only when `fullyIdle` is true |
...
| **DeepSeek Harness** | Supported | Hook integration via `~/.dsh/profiles/web/cordis.patch.yml` config-driven hooks (`turn/start`, `turn/end`), process discovery for `dsh web` sessions |
